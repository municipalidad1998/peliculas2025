...
/** DeTodoPeliculas - detodopeliculas.nu */
class DeTodoPeliculasProvider : MovieProvider() {
    override var mainUrl = "https://detodopeliculas.nu"
    override var name = "DeTodoPeliculas"
    ...
    override suspend fun getMainPage(page: Int, request: HomePageRequest): HomePageList? {
        ...
        val items = doc.select("article, .movie-card, .card, div[class*=item]").mapNotNull { el ->
            ...
            movieSearchResponse(t.trim(), resolveUrl(mainUrl, h), fixUrl(img), y)
        }
        return HomePageList("DeTodoPeliculas", listOf(HomePageListResponse("Peliculas y Series", items)))
    }

    override suspend fun search(query: String): List<SearchResponse> {
        val doc = safeDocument("$mainUrl/?s=${enc(query)}") ?: throw Exception("Search failed")
        return doc.select("article, .movie-card, .card, div[class*=item]").mapNotNull { el ->
            val t = el.selectFirst("h2, h3, .title")?.text() ?: return@mapNotNull null
            val h = el.selectFirst("a")?.attr("href") ?: return@mapNotNull null
            val img = el.selectFirst("img")?.attr("src") ?: el.selectFirst("img")?.attr("data-src")
            movieSearchResponse(t.trim(), resolveUrl(mainUrl, h), fixUrl(img))
        }
    }
    ...
    override suspend fun load(url: String): LoadResponse? {
        ...
        val poster = fixUrl(doc.ogImage() ?: doc.selectFirst(".poster img, img")?.attr("src"))
        val plot = doc.ogDescription() ?: doc.selectFirst(".description, .sinopsis, .content")?.text()
        val y = Regex("(19|20)\\d{2}").find(doc.selectFirst(".year, .date, time")?.text() ?: "")?.value?.toIntOrNull()
        ...
        return if (eps.isEmpty()) newMovieLoadResponse(title.trim(), url) { this.posterUrl = poster; this.plot = plot; this.year = y; this.tags = genres }
        else newTvSeriesLoadResponse(title.trim(), url, TvType.TvSeries, eps) { this.posterUrl = poster; this.plot = plot; this.year = y; this.tags = genres }
    }
