...
/** AnimeJara - animejara.com */
class AnimeJaraProvider : AnimeProvider() {
    ...
    override suspend fun getMainPage(page: Int, request: HomePageRequest): HomePageList? {
        val doc = safeDocument("$mainUrl/inicio") ?: return null
        val items = doc.select("article, .anime-card, div[class*=card]").mapNotNull { el ->
            val t = el.selectFirst("h2, h3, .title, a.title")?.text() ?: return@mapNotNull null
        ...
        return HomePageList("AnimeJara", listOf(HomePageListResponse("Anime", items)))
    }

    override suspend fun search(query: String): List<SearchResponse> {
        val doc = safeDocument("$mainUrl/buscar?q=${enc(query)}") ?: throw Exception("Search failed")
        return doc.select("article, .anime-card, div[class*=card]").mapNotNull { el ->
    ...
    override suspend fun load(url: String): LoadResponse? {
        ...
        val genres = doc.select(".genre a, .generos a, .category a").map { it.text().trim() }
        val eps = doc.select("ul.episodes li a, .episode-list a, .cap-list a, a[href*=capitulo], a[href*=episode]").mapNotNull { a ->
            val href = a.attr("href"); if (href.isBlank()) return@mapNotNull null
            val num = Regex("(?:Ep|Cap|Season\\s*\\d+x)(\\d+)").find(a.text())?.groupValues?.get(1)?.toIntOrNull()
