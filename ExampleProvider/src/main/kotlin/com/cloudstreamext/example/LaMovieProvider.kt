...
/** LaMovie - lamovie.org */
class LaMovieProvider : MovieProvider() {
    ...
    override suspend fun getMainPage(page: Int, request: HomePageRequest): HomePageList? {
        ...
        val items = doc.select("article, .movie-card, .card, div[class*=post], div[class*=item]").mapNotNull { el ->
        ...
        return HomePageList("LaMovie", listOf(HomePageListResponse("Películas", items)))
    }
    ...
    override suspend fun loadLinks(data: String, isCasting: Boolean, subtitleCallback: (SubtitleFile) -> Unit, callback: (ExtractorLink) -> Unit): Boolean {
        ...
        doc.select("iframe[src]").forEach { i ->
            val src = i.attr("src"); if (src.isNotBlank() && !src.contains("google") && !src.contains("facebook")) {
