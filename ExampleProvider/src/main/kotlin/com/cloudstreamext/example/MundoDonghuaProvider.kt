...
/** MundoDonghua - mundodonghua.com - Animación China */
class MundoDonghuaProvider : AnimeProvider() {
    ...
    override suspend fun getMainPage(page: Int, request: HomePageRequest): HomePageList? {
        ...
        val items = doc.select("article, .donghua-card, .card, div[class*=item], div[class*=post]").mapNotNull { el ->
            val t = el.selectFirst("h2, h3, .title, .entry-title, a.title")?.text() ?: return@mapNotNull null
        ...
        return HomePageList("MundoDonghua", listOf(HomePageListResponse("Donghua", items)))
    }

    override suspend fun search(query: String): List<SearchResponse> {
        val doc = safeDocument("$mainUrl/?s=${enc(query)}") ?: throw Exception("Search failed")
        return doc.select("article, .donghua-card, .card, div[class*=item]").mapNotNull { el ->
    ...
    override suspend fun load(url: String): LoadResponse? {
        ...
        val eps = doc.select("ul.episodes li a, .episode-list a, .cap-list a, a[href*=episodio]").mapNotNull { a ->
            val href = a.attr("href"); if (href.isBlank()) return@mapNotNull null
            val num = Regex("Episodio\\s*(\\d+)").find(a.text())?.groupValues?.get(1)?.toIntOrNull()
