...
/** StreamXHD - stream-xhd.com - Live Sports */
class StreamXHDProvider : LiveTVProvider() {
    ...
    override suspend fun getMainPage(page: Int, request: HomePageRequest): HomePageList? {
        ...
        val items = doc.select("article, .event-card, .card, div[class*=event], div[class*=item]").mapNotNull { el ->
            val t = el.selectFirst("h2, h3, .title, .event-title")?.text() ?: return@mapNotNull null
        ...
        return HomePageList("StreamXHD", listOf(HomePageListResponse("Deportes en Vivo", items)))
    }
