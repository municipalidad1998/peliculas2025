...
/** TlNovelas - ww2.tlnovelas.net */
class TlNovelasProvider : MovieProvider() {
    ...
    override suspend fun getMainPage(page: Int, request: HomePageRequest): HomePageList? {
        ...
        val items = doc.select("article, .novela-card, .card, div[class*=item], div[class*=post]").mapNotNull { el ->
        ...
        return HomePageList("TlNovelas", listOf(HomePageListResponse("Novelas", items)))
    }

    override suspend fun search(query: String): List<SearchResponse> {
        val doc = safeDocument("$mainUrl/?s=${enc(query)}") ?: throw Exception("Search failed")
        return doc.select("article, .novela-card, .card, div[class*=item]").mapNotNull { el ->
    ...
    override suspend fun load(url: String): LoadResponse? {
        ...
        return if (eps.isEmpty()) newMovieLoadResponse(title.trim(), url) { this.posterUrl = poster; this.plot = plot }
        else newTvSeriesLoadResponse(title.trim(), url, TvType.TvSeries, eps) { this.posterUrl = poster; this.plot = plot }
    }
