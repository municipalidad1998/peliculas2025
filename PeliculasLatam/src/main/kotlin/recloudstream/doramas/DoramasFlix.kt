...
class DoramasFlix : BaseSiteProvider() {
    ...
    override suspend fun getMainPage(page: Int, request: MainPageRequest): HomePageResponse {
        ...
        val results = doc.select("article, div[class*=dorama], .poster, div[class*=item]").mapNotNull { el ->
        ...
        return newHomePageResponse(listOf(HomePageList("Doramas", results, true)))
    }

    override suspend fun search(query: String, page: Int): SearchResponseList? {
        val doc = app.get("$mainUrl/?s=${query.encodeUri()}").document
        return doc.select("article, div[class*=dorama], .poster").mapNotNull { el ->
