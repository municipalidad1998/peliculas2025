...
class MundoDonghua : BaseSiteProvider() {
    ...
    override suspend fun getMainPage(page: Int, request: MainPageRequest): HomePageResponse {
        ...
        val results = doc.select("article, div[class*=donghua], .poster, div[class*=anime]").mapNotNull { el ->
        ...
        return newHomePageResponse(listOf(HomePageList("Donghua", results, true)))
    }

    override suspend fun search(query: String, page: Int): SearchResponseList? {
        val doc = app.get("$mainUrl/buscar/${query.encodeUri()}/").document
        return doc.select("article, div[class*=donghua], .poster").mapNotNull { el ->
